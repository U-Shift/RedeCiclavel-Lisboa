...
slider <- column(9, shinyWidgets::sliderTextInput(
  inputId = "Ano", "Ano:",
...
server <- function(input, output) {
  ...
  output$map <- renderLeaflet({
    leaflet() |>
      ...
      addLayersControl(
        overlayGroups = c("Ciclovias", "30+Bici ou Não dedicada", "Percurso Ciclo-pedonal"),
        baseGroups = c("mapa", "satélite"),
        options = layersControlOptions(collapsed = F)
      ) |>
      hideGroup(c("Percurso Ciclo-pedonal")) |>
      addLegend(
        position = "bottomright", colors = c("#1A7832", "#AFD4A0"),
  ...
  observe({
    leafletProxy("map") |>
      ...
      addPolylines(
        data = CICLOVIAS[CICLOVIAS$AnoT == input$Ano &
        ...
        label = ~DESIGNACAO,
        popup = sprintf(
          "<strong>%s</strong><br>%s<br>Ano: %s<br>Extensão: %s metros", # ver o pq do %s ou %g: https://rdrr.io/r/base/sprintf.html
  ...
  grafico <- ggplotly(grafico) |>
    ...
    layout(
      legend = list(orientation = "h", y = 1.1, x = 0.05, traceorder = "reversed"),
      hovermode = "x"
    ) |> # para aparecer legenda logo em ambos
    style(hoverinfo = "y")
